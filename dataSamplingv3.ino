...
const int SAMPLE_SIZE = 5; // set before use
...
DataPoint sumSq = {0}; // Initialize to 0 new
File dataFile;
...
void setup() {
  ...
  dataFile.println("Time (s), Temp (C), stdev (C), Humidity (%RH), stdev (%RH) ");
  dataFile.close();
...
void loop() {
  ...
  if (n < 20 || (abs(tempC - mean.tempC / n) <= 2 * DSATEMPC) && (abs(humidity - mean.humidity / n) <= 2 * DSAHUMIDITY)) { // skip outliers
    if (n < SAMPLE_SIZE) {
      ...
      sumSq.tempC += tempC * tempC; //new
      sumSq.humidity += humidity * humidity; //new
      n++;
    } else {
      mean.tempC /= n;
      mean.humidity /= n;
      ...
      stdev.tempC = sqrt((sumSq.tempC / n) - (mean.tempC * mean.tempC));
      stdev.humidity = sqrt((sumSq.humidity / n) - (mean.humidity * mean.humidity));
